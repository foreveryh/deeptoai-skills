...
| **mdx-validator** | MDX 语法预检查 | 每次翻译后 |
| **fumadocs-deploy** | 构建部署验证 | 每次发布 |
...
外部 URL
    ↓
fumadocs-article-importer  ──→  下载文章 + 图片（使用安全文件名）
    ↓
article-translator  ──────────→  翻译到 en/zh/fr
    ↓                            应用 MDX 安全规则
mdx-validator  ──────────────→  预检查语法错误
    ↓                            自动修复
fumadocs-deploy  ─────────────→  构建并部署
    ↓                            验证所有资源
完成！
